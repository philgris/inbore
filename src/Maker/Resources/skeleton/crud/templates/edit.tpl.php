...
{% block stylesheets %}
	...
	{# Load for specific CSS with this TWIG call : {{ encore_entry_link_tags("<?= strtolower($entity_class_name) ?>-form") }} 
         ...
         .addStyleEntry('contact-form', './assets/forms/Core/css/<?= strtolower($entity_class_name) ?>-form.css') 
        #}
{% endblock %}
...
{% block action %}
	{{parent()}}
        {# Add a button Back to linked entity : name_of_linked_entity
            {% if edit_form.vars.data.name_of_linked_entityFk is not null %}
                    <a href="{{path('name_of_linked_entity_show', {id: edit_form.vars.data.name_of_linked_entityFk.id})}}" class="btn btn-sm btn-info">
...
{% block scripts %}
	{{ parent() }}
        {# Load for specific js script with the TWIG call : {{ encore_entry_script_tags("<?= strtolower($entity_class_name) ?>-form") }} 
         ...
         .addEntry('<?= strtolower($entity_class_name) ?>-form', './assets/forms/Core/js/<?= strtolower($entity_class_name) ?>-form.js') 
        #}
{% endblock %}
