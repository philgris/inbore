...
// InBORe Repository generation
    ...
    public function editActionBeforeFormIsSubmitted(Request $request, Adress $adress)
    {
        ...
        */        
        ...
        // Set the array(s) collection(s) of the Adress entity before the handleRequest action (before the form is submited)
        $this->list_of_collection_embed = [];
        foreach (self::LIST_OF_COLLECTIONS_EMBEDED as $name_of_array_collection) {
            $this->list_of_collection_embed [$name_of_array_collection] = $this->service->setArrayCollection($name_of_array_collection, $adress);
        }
        //       
        return $edit_form_parameters ;
    }
    
    public function editActionAfterFormIsSubmitted(Request $request, Adress $adress)
    {
        // Del the array(s) collection(s) of the Contact entity after the handleRequest and the form ois submited and  validated 
         foreach (self::LIST_OF_COLLECTIONS_EMBEDED as $name_of_array_collection) {
           $this->service->DelArrayCollection($name_of_array_collection, $adress, $this->list_of_collection_embed [$name_of_array_collection]);
        }
    }
       ...
       */
       ...
       // CASE where idFk and nameFk are defined
        if ($idFk != null && $nameFk != null) {
            if ($nameFk == 'core'.ucfirst(self::NAME_MAIN_LINKED_TABLE).'Fk')  $where .= " AND adress.".self::NAME_FK_TO_MAIN_LINKED_TABLE." = '". $idFk. "'"  ;
        }
       
       
       /** // Add  LEFT JOIN section if needed : 
       $leftJoin .= ' LEFT JOIN nameEntityRel ON adress.nameEntityRel_fk = nameEntityRel.id ' ;
       */
